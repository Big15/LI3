...
Q10s* travessia_tree_q10_fact(const struct avl_node *node, Q10s* res, int n) {
    ...
    res = q10_add_list(res, get_codigo(old->cod), t, n, old->vt1, old->vt2, old->vt3);
...
List q10_fact(Fact fact, List list, int n){
    ...
    while(res != NULL && i < n){
        sprintf(aux, "%s", get_codigo(res->cod));
        list = add_string_l(list, aux);
        sprintf(aux, "Total    Filial1   Filial2   Filial3  NºClientes");
        list = add_string_l(list, aux);
